...
*/
#ifndef BLFIELDMAP_HH
#define BLFIELDMAP_HH
 ...
 *
 *	NOTE: this class implements Bz, Br, and Bphi for a cylindrical
 *	map created via createCylerndarMap(). It only supports Bz and Br
 *	when read from a file. When writing to a file, Bphi is ignored.
 *
 *
 *	Input File format:
...
public:
	...
	/// (elements using BLFieldMap perform the global->local conversion)
	void getFieldValue(const G4double local[4], G4double field[6],
			G4double current=1.0, G4double gradient=1.0);

	/// getFieldValueNoTimeNoScale() as above, except no time dependence
	/// is applies, and no scaling is performed.
	void getFieldValueNoTimeNoScale(const G4double local[4], 
			G4double field[6]);
	...
	/// This version computes Bz, Br, Bphi, Ez, Er.
	/// returns false if error.
