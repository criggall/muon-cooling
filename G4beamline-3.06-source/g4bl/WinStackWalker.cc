...
class WinStackWalkerInternal
{
public:
  WinStackWalkerInternal(WinStackWalker *parent, HANDLE hProcess)
  {
    m_parent = parent;
    m_hDbhHelp = NULL;
    pSC = NULL;
  ...
  BOOL Init(LPCSTR szSymPath)
  {
    ...
    if (this->pSGSP != NULL)
    {
      if (this->pSGSP(m_hProcess, buf, WinStackWalker::STACKWALK_MAX_NAMELEN) == FALSE)
        this->m_parent->OnDbgHelpErr("SymGetSearchPath", GetLastError(), 0);
    }
    char szUserName[1024] = "NONE";
    DWORD dwSize = 1024;
    //TJR GetUserNameA(szUserName, &dwSize);
...
WinStackWalker::WinStackWalker(DWORD dwProcessId, HANDLE hProcess)
{
  ...
  this->m_sw = new WinStackWalkerInternal(this, this->m_hProcess);
...
static WinStackWalker::PReadProcessMemoryRoutine s_readMemoryFunction = NULL;
...
BOOL WinStackWalker::ShowCallstack(HANDLE hThread, const CONTEXT *context, PReadProcessMemoryRoutine readMemoryFunction, LPVOID pUserData)
{
  ...
  WinStackWalkerInternal::IMAGEHLP_MODULE64_V2 Module;
