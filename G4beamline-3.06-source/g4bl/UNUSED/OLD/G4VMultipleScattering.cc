...
{
printf("G4VMultipleScattering::AlongStepGetPhysicalInteractionLength Entered %.6f,%.6f,%.6f\n",track.GetPosition().x(),track.GetPosition().y(),track.GetPosition().z());
  ...
  *selection = NotCandidateForSelection;
  ...
  // select new model
  if(1 < numberOfModels) {
printf("AlongStepGPIL selecting model index=%d\n",track.GetMaterialCutsCouple()->GetIndex());
...
printf("AlongStepGPIL MMM tPathLength=%.8f physStepLimit=%.8f\n",tPathLength,physStepLimit);
  } else { isActive = false; }
  ...
  //*/
printf("G4VMultipleScattering::AlongStepGetPhysicalInteractionLength returns gPathLength=%.6f\n",gPathLength);
  return gPathLength;
}
...
G4VMultipleScattering::AlongStepDoIt(const G4Track& track, const G4Step& step)
{
  ...
  fPositionChanged = false;
printf("G4VMultipleScattering::AlongStepDoIt Entered %.6f,%.6f,%.6f\n",fNewPosition.x(),fNewPosition.y(),fNewPosition.z());
...
printf("G4VMultipleScattering::AlongStepDoIt calling ComputeTrueStepLength: range=%.6f geomLength=%.6f\n",range,geomLength);
    tPathLength = currentModel->ComputeTrueStepLength(geomLength);
printf("G4VMultipleScattering::AlongStepDoIt XXX tPathLength=%.6f\n",tPathLength);
...
printf("G4VMultipleScattering::AlongStepDoIt YYY tPathLength=%.6f\n",tPathLength);
...
printf("G4VMultipleScattering::AlongStepDoIt AAA %.6f,%.6f,%.6f\n",fNewPosition.x(),fNewPosition.y(),fNewPosition.z());
	  ...
	  if(dispR < postSafety) {
	    fNewPosition += displacement;
printf("G4VMultipleScattering::AlongStepDoIt BBB %.6f,%.6f,%.6f\n",fNewPosition.x(),fNewPosition.y(),fNewPosition.z());
...
printf("G4VMultipleScattering::AlongStepDoIt CCC %.6f,%.6f,%.6f\n",fNewPosition.x(),fNewPosition.y(),fNewPosition.z());
		} else { 
	      ...
	      { 
		fNewPosition += displacement*(postSafety/dispR - 1.0); 
printf("G4VMultipleScattering::AlongStepDoIt DDD %.6f,%.6f,%.6f\n",fNewPosition.x(),fNewPosition.y(),fNewPosition.z());
	      }
	    // reduced displacement
	  } else if(postSafety > geomMin) {
	    fNewPosition += displacement*(postSafety/dispR); 
printf("G4VMultipleScattering::AlongStepDoIt EEE %.6f,%.6f,%.6f\n",fNewPosition.x(),fNewPosition.y(),fNewPosition.z());

	    // very small postSafety
	  } else {
	    fPositionChanged = false;
printf("G4VMultipleScattering::AlongStepDoIt fPositionChanged = false\n");
	  }
	}
...
{
printf("G4VMultipleScattering::PostStepDoIt Entered %.6f,%.6f,%.6f\n",track.GetPosition().x(),track.GetPosition().y(),track.GetPosition().z());
