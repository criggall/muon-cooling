...
printf("G4SteppingManager::InvokeAlongStepDoItProcs after %s.AlongStepDoIt %.6f,%.6f,%.6f\n",fCurrentProcess->GetProcessName().c_str(),fTrack->GetPosition().x(),fTrack->GetPosition().y(),fTrack->GetPosition().z());
...
printf("G4SteppingManager::InvokePSDIP after %s.PostStepDoIt %.6f,%.6f,%.6f\n",fCurrentProcess->GetProcessName().c_str(),fTrack->GetPosition().x(),fTrack->GetPosition().y(),fTrack->GetPosition().z());
	 ...
	 fParticleChange->UpdateStepForPostStep(fStep);
         ...
         fStep->UpdateTrack();
printf("G4SteppingManager::InvokePSDIP after UpdateTrack %s.PostStepDoIt %.6f,%.6f,%.6f\n",fCurrentProcess->GetProcessName().c_str(),fTrack->GetPosition().x(),fTrack->GetPosition().y(),fTrack->GetPosition().z());
		...
		delete tempSecondaryTrack;
         ...
         // clear ParticleChange
         fParticleChange->Clear();
printf("G4SteppingManager::InvokePSDIP returns %.6f,%.6f,%.6f\n",fTrack->GetPosition().x(),fTrack->GetPosition().y(),fTrack->GetPosition().z());
}
