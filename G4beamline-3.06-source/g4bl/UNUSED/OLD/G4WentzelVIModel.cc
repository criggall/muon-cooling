...
{
printf("G4WentzelVIModel::ComputeTrueStepLength Entered geomStepLength=%.6f\n",geomStepLength);
...
{
printf("G4WentzelVIModel::ComputeTransportXSectionPerVolume Entered cosTheta=%.6f\n",cosTheta);
...
printf("WentzelVI::CTXSPV cosTetMaxNuc=%.8f cosTheta=%.8f\n",cosTetMaxNuc,cosTheta);
  ...
  G4double cut = (*currentCuts)[currentMaterialIndex];
  if(fixedCut > 0.0) { cut = fixedCut; }
printf("WentzelVI::CTXSPV cut=%.8f\n",cut);
...
printf("QQQ nucsec=%.6f esec=%.6f xtsec=%.6f\n",nucsec,esec,xtsec);
    }
    xsecn[i] = xtsec;
    prob[i]  = esec;
...
printf("G4WentzelVIModel::ComputeTransportXSectionPerVolume returns xs=%.6f\n",xs);
  return xs;
}
