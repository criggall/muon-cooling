...
{
printf("G4ProcessManager::StartTracking Entered %.6f,%.6f,%.6f\n",aTrack->GetPosition().x(),aTrack->GetPosition().y(),aTrack->GetPosition().z());
  for (G4int idx = 0; idx<theProcessList->entries(); idx++){
...
{
printf("G4ProcessManager::EndTracking\n");
  for (G4int idx = 0; idx<theProcessList->entries(); idx++){
    if (GetAttribute(idx)->isActive)
